...
def create_design_matrix(x,y,d):
    """Creating design matrix for 2d polynomial. d is the degree of
    the polynomial. The design matrix has size (n*n, p), where
    p = (d+1)(d+2)/2"""
...
def train_test_np(X, y, test_ratio=0.2):
    """Splitting data into train and test, and then scaling it as well. Returns
    x_train, x_test, y_train, y_test. The test ratio is set to 20% by
    default, if you want to change it, you can input it as an argument"""
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_ratio, random_state=seed)
    X_train = scaling(X_train) ;  X_test = scaling(X_test)
    return X_train, X_test, y_train, y_test

def to_categorical_numpy(integer_vector):
    """Returns the one-hot vectors for classification problems"""
...
def accuracy_score_numpy(Y_test, Y_pred):
    """Measure of error when we are doing classification problems.
    Counts how many classes were predicted correctly on the data"""
